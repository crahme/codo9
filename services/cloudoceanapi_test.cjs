...
import {CloudOceanAPI} from './cloudoceanapi.cjs';
...
async function main() {
  ...
  if (!moduleUuid || !measuringPointUuid) {
    console.error('Usage: node services/cloudoceanapi_test.js <moduleUuid> <measuringPointUuid> [start: YYYY-MM-DD] [end: YYYY-MM-DD]');
    process.exit(1);
  }
