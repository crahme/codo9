...
// Simple logger (you can replace with a real logger later)
const logger = {
...
export class CloudOceanService {
  constructor() {
    ...
    this.headers = {
      "Access-Token": process.env.API_KEY?.trim(),
  ...
  async getConsumptionData(startDate, endDate, limit = 50, offset = 0) {
    ...
    try {
      ...
      let totalStations = measuringPoints.length;
        ...
        try {
          ...
          if (Array.isArray(data) && data.length > 0) {
            ...
            const consumption = Math.max(
              0,
              sortedReads[sortedReads.length - 1].cumulative_kwh -
                sortedReads[0].cumulative_kwh
            );
            ...
            consumptionData.push({
              ...
              consumption,
              firstReading: sortedReads[0],
              lastReading: sortedReads[sortedReads.length - 1],
            ...
            processedStations++;
            logger.info(
              `Successfully processed ${point.name}: ${consumption.toFixed(
                2
              )} kWh`
            );
          }
          ...
          logger.error(
            `Failed to fetch data for ${point.name}: ${error.message}`
          );
          continue;
        }
      }
      ...
      if (consumptionData.length === 0) {
        throw new Error("Failed to fetch data for all measuring points");
      }

      logger.info(
        `Successfully fetched data for ${consumptionData.length}/${totalStations} stations`
      );
...
// 🏃 Runner: executes only if you run this file directly
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  (async () => {
    try {
      ...
      console.error("❌ Error running service:", err.message);
    }
  })();
}
